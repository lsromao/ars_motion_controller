...
class ArsMotionControllerRos:
  ...
  def open(self):
    ...
    # Publisher
    # Robot cmd stamped
    self.robot_vel_cmd_stamped_pub = rospy.Publisher('robot_cmd_stamped', TwistStamped, queue_size=1)
    # Robot cmd
    self.robot_vel_cmd_pub = rospy.Publisher('robot_cmd', Twist, queue_size=1)
  ...
  def robotCmdPub(self):
    ...
    if(self.robot_vel_cmd_pub):
      self.robot_vel_cmd_pub.publish(robot_velo_cmd_stamped_msg.twist)
